...
//       try {
//         const response = await axios.get("http://localhost:8080/categories"); // Đổi URL API nếu cần
...
//           <input {...register("name", {required: true})} type="text" placeholder="Tên phim"
//                  className="w-full p-2 border rounded"/>
//           <input {...register("director")} type="text" placeholder="Đạo diễn" className="w-full p-2 border rounded"/>
//           <input {...register("actor")} type="text" placeholder="Diễn viên" className="w-full p-2 border rounded"/>
//           <textarea {...register("description")} placeholder="Mô tả phim"
//                     className="w-full p-2 border rounded"></textarea>
//           <input {...register("duration", {required: true})} type="number" placeholder="Thời lượng (phút)"
//                  className="w-full p-2 border rounded"/>
//           <input {...register("releaseDate")} type="datetime-local" className="w-full p-2 border rounded"/>
//
//           <input
//               {...register("ageLimit")}
//               type="number"
...
//           <input type="file" accept="image/*" onChange={(e) => uploadImage(e.target.files[0], "posterUrl")}
//                  className="w-full p-2 border rounded"/>
//
//           {/* Upload Banner */}
//           <label className="block font-semibold">Banner:</label>
//           <input type="file" accept="image/*" onChange={(e) => uploadImage(e.target.files[0], "bannerUrl")}
//                  className="w-full p-2 border rounded"/>
//
//           {/* Upload Thumbnail Trailer */}
//           <label className="block font-semibold">Thumbnail Trailer:</label>
//           <input type="file" accept="image/*" onChange={(e) => uploadVideo(e.target.files[0], "thumbnailUrl")}
//                  className="w-full p-2 border rounded"/>
//
//
...
export default function AddMovieForm() {
  ...
  const onSubmit = async (data) => {
    const formattedData = {
      ...data,
      category: { categoryId: data.categoryId },
      isDelete: false // Thêm trường isDelete với giá trị mặc định là false
    };
  ...
  return (
      <div className="max-w-lg mx-auto p-6 bg-black shadow-md rounded-lg">
        <h2 className="text-2xl font-semibold text-center mb-4">Thêm Phim Mới</h2>
        {message && (
            <p className={`text-center ${message.includes("thất bại") ? "text-red-600" : "text-green-600"}`}>
              {message}
            </p>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <button
              type="submit"
              disabled={uploading}
              className={`w-full ${uploading ? 'bg-gray-500' : 'bg-blue-600 hover:bg-blue-700'} text-white p-2 rounded`}
          >
            {uploading ? "Đang tải..." : "Thêm Phim"}
          </button>
        </form>
